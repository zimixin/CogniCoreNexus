...
Ключевой принцип: **LLM — исполнитель, а не мозг**. Мозг — это L0-L3 пробуждение памяти, логический вывод и матрицы рассуждений. LLM получает уже дистиллированный контекст и отвечает на его основе.
...
```
query → L0 (точное совпадение) → если ≥0.7 → ответ
  ↓ (нет точного)
L1 (AAAK-ключи) → если ≥0.85 → ответ
  ↓ (низкая уверенность)
L2 (семантика + гены) → если ≥0.75 + vectors → ответ
  ↓
L3 (BFS по графу комнат) → всегда возвращает
  ↓
контекст → логический вывод → когнитивная матрица → LLM → сохранение
```

### Ранняя остановка (L0-L3)

Система не проходит все 4 уровня каждый раз. Как только на уровне достигается достаточная уверенность (порог `search_threshold`, по умолчанию 0.7), поиск останавливается. Это экономит ресурсы при частых/точных запросах.
...
- Желательно: `pip install requests` (для LLM)
...
|---------|----------|--------|
| `/query <текст>` | Основной запрос к системе | `/query что такое SOLID?` |
...
>>> /query что такое SOLID?

[Обработка: что такое SOLID?...]

--- ОТВЕТ ---
[Символьный режим — LLM не подключена]

Найдено в пространственной памяти:
  • seed_entry_0
    (LOCI_MEMO seed_entry_0 :content "CogniCore Nexus v2.0 — стабильная версия от 2026-05-25" :tags (LIST seed) :time "2026-05-25T08:00:00+0000")
...
--- ТРАССИРОВКА (7 шагов) ---
  [L0] Точное совпадение...
    L0: найдено 0 результатов
  [L1] Поиск по ключам AAAK...
    L1: найдено 0 результатов
  [L2] Семантический поиск + гены...
    L2: 0 локусов + 1 генов
  [L3] Полный обход графа локусов (BFS)...
    L3: найдено 1 результатов
  Контекст: 1 локусов, 1 генов
  [Logic] Применение правил вывода...
  Выведено 0 новых фактов
  [Matrix] Выбор когнитивной матрицы...
    Запущена матрица: general_inquiry
    Матрица выполнена: completed
  [LLM] LLM не подключена — символьный режим

Уверенность: 0.60
LLM: нет (символьный режим)
...
- Ядро process_query: полный цикл
- MCP протокол: in-memory SDK транспорт
